...
export const useCommunityStore = defineStore('communityStore', {
  state: () => ({
    ...
    authUserCommunities: [] as Array<ICommunityPreview>,
  ...
  getters: {
    ...
    _communityList: (state: any) => state.communityList as Array<ICommunityPreview>,
    _userCommunities: (state: any) => state.userCommunities as Array<ICommunityPreview>,
    _authUserCommunities: (state: any) => state.authUserCommunities as Array<ICommunityPreview>,
  ...
  actions: {
    ...
    //GET USER COMMUNITIES
    async getAuthUserCommunities(id: string) {
      try {
        const res = await instance.get(`/community/user-communities?id=${id}`)
        // console.log(res.data.data);
        this.authUserCommunities = res.data.data
      } catch (error: any) {
        console.error(error.message)
      }
    },
    ...
    async deleteCommunity(communityId: string) {
      try {
        const res = await instance.post(`/Community/Delete?id=${communityId}`, {})
        this.statusCode = res.data.statusCode
        if (this.statusCode === 200) {
          this.getUserCommunities(useAuthStore()._user.id)
        }
        setTimeout(() => {
          this.statusCode = 0
        }, 3000)
    ...
    async deleteCommunityPermanently(communityId: string) {
      try {
        const res = await instance.post(`/Community/DeletePermanently/${communityId}`, {})
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async assignUserAsModerator(user: object) {
      try {
        const res = await instance.post('/Community/AssignUserAsModerator', user)
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async getCommunityParticipiants(communityId: string) {
      try {
        const res = await instance.get(`/community/${communityId}/Participiants`)
        // console.log(res.data)
    ...
    async getJoinRequests(communityId: FormData) {
      try {
        const res = await instance.get('/Community/Join-Requests?skip=0&take=5')
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async searchParticipiants(communityId: string, text: string) {
      try {
        const res = await instance.get(`/community/${communityId}/Participiants/Search?q=${text}`)
        // console.log(res.data)
        this.searchedParticipiants = res.data.data
      } catch (error: any) {
        console.error(error.message)
    ...
    async updateCoverImage(communityId: string, data: FormData) {
      try {
        const res = await instance.post(`/community/${communityId}/update-cover-image`, data)
        // console.log(res.data)
        this.searchedParticipiants = res.data.data
      } catch (error: any) {
        console.error(error.message)
    ...
    async removeCoverImage(communityId: string) {
      try {
        const res = await instance.post(`/community/${communityId}/remove-cover-image`, {})
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async updateBannerImage(communityId: string, data: FormData) {
      try {
        const res = await instance.post(`/community/${communityId}/update-banner-image`, data)
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async removeBannerImage(communityId: string) {
      try {
        const res = await instance.post(`/community/${communityId}/remove-banner-image`, {})
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
    ...
    async kickUser(communityId: string, targetId: string) {
      try {
        const res = await instance.post(`/community/kick-user/${communityId}/${targetId}`, {})
        // console.log(res.data)
      } catch (error: any) {
        console.error(error.message)
      }
