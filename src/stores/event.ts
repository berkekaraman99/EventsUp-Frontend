...
export const useEventStore = defineStore('eventStore', {
  ...
  getters: {
    ...
    _eventAttendees: (state: any) => state.eventAttendees as Array<IEventAttend>
  },
  actions: {
    ...
    async deleteEventCompletely(eventId: string) {
      try {
        const res = await instance.post(`/event/delete-completely?id=${eventId}`, {})
        // console.log(res.data);
        this.statusCode = res.data.statusCode
        setTimeout(() => {
          this.statusCode = 0
    ...
    async deleteComment(commentId: string, eventId: string) {
      try {
        const res = await instance.post(`/event/comment/delete/${commentId}`, {})
        // console.log(res.data);
        this.statusCode = res.data.statusCode
        setTimeout(() => {
          this.statusCode = 0
