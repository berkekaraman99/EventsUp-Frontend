      ...
      <div class="card-body">
        ...
        <div>
          ...
          >
            ...
            <span class="ms-2 text-primary">{{ t('community.changecoverimage') }}</span>
          </div>
        </div>
        <div class="collapse" id="collapseChangeCoverImage">
          <div class="card card-body">
            <FormKit type="form" @submit="changeCoverImage" submit-label="Create" :actions="false">
              <FormKit
                type="file"
                label="Files"
              ...
              <div class="d-flex align-items-center">
                <FormKit
                  type="submit"
                  :label="
                    loading ? 'Loading' : statusCode !== 200 ? 'Kapak Resmini Değiştir' : 'Başarılı'
                  "
                ...
                <FormKit
                  type="button"
                  :label="
                    loading ? 'Loading' : statusCode !== 200 ? 'Kapak Resmini Kaldır' : 'Başarılı'
                  "
        ...
        <div>
          ...
          >
            <i class="fa-solid fa-pen-to-square"></i>
            <span class="ms-2 text-primary">{{ t('community.changebannerimage') }}</span>
          </div>
        </div>
        <div class="collapse" id="collapseChangeBannerImage">
          <div class="card card-body">
            <FormKit type="form" @submit="changeBannerImage" submit-label="Create" :actions="false">
              <FormKit
                type="file"
                label="Files"
              ...
              <div class="d-flex align-items-center">
                <FormKit
                  type="submit"
                  :label="loading ? 'Loading' : statusCode !== 200 ? 'Afişi Değiştir' : 'Başarılı'"
                  wrapper-class="text-center"
                  :classes="{ input: '' }"
                  :disabled="loading || statusCode === 200"
                />
                <FormKit
                  type="button"
                  :label="loading ? 'Loading' : statusCode !== 200 ? 'Afişi Kaldır' : 'Başarılı'"
...
const { _statusCode: statusCode, _community: community } = storeToRefs(communityStore)

const changeCoverImage = async () => {
  const body = new FormData()
  body.append('File', coverImage.value)
  await communityStore.updateCoverImage(community.value.id, body).then(async () => {
    await communityStore.getCommunity(community.value.id)
  })
}

const removeCoverImage = async () => {
  await communityStore
    .removeCoverImage(community.value.id)
    .then(async () => await communityStore.getCommunity(community.value.id))
}

const changeBannerImage = async () => {
  const body = new FormData()
  body.append('File', bannerImage.value)
  await communityStore.updateBannerImage(community.value.id, body).then(async () => {
    await communityStore.getCommunity(community.value.id)
  })
}
...
const removeBannerImage = async () => {
  await communityStore
    .removeBannerImage(community.value.id)
    .then(async () => await communityStore.getCommunity(community.value.id))
}

const deleteCommunity = async () => {
  changeLoadingState()
  await communityStore.deleteCommunity(community.value.id).then(() => {
    changeLoadingState()
